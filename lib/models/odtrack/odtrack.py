    ...
    def forward(self, template: torch.Tensor,
        ...
        for i in range(len(search)):
            ...
            if self.backbone.add_cls_token:
                self.track_query = (x[:, len_m:len_m+self.token_len].clone()).detach() # stop grad  (B, N, C)
##########################################
...
def build_odtrack(cfg, training=True):
    ...
    # print("666666666666",box_head.box)
    backbone.aa = box_head.box
